...
Theorem bag_theorem : forall (n: nat) (s: bag), count n (add n s) = S (count n s).
Proof.
  intros n s. induction s as [|h t].
...
Proof.
  intros f Inv. intros n m. intros E. rewrite -> Inv. rewrite <- E. rewrite <- Inv. reflexivity.
Qed.
(** [] *)
...
(** This solution is not so good: If [nth_bad] returns [42], we
    ...
    without further processing. A be
tter alternative is to change the
